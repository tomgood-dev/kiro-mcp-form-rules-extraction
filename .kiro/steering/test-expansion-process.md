...
8. **Inputs must be provably correct by construction, and assertions strict.** A test whose inputs don't actually reach the state the AC describes can pass for the wrong reason (the AC12 bug: values borrowed from a sibling test summed to $80k when the AC needed >$250k, so the min-premium rule fired first). For every threshold/scenario test verify all six: (1) threshold values derived from THIS AC's own numbers with the arithmetic shown in a comment — never borrowed by analogy; (2) no *other* rule can fire first with these inputs (keep untested dimensions in a clean range); (3) every Given/When clause satisfied (age band, gender, occupation, cover combo, premium structure); (4) all precondition state established; (5) correct field/index when several similar controls exist; (6) **assert the EXACT expected message/value and treat a *different* result as a FAILURE, never "some error appeared"** — this strict assertion is the backstop that catches any wrong input the first five miss. Checks 1-5 prevent; check 6 guarantees detection. Not claimed exhaustive — extend the list when a new wrong-input class is found.
