...
## 4. Known discrepancies between testing sessions — read before trusting any single source

Two independent testing sessions (2026-08-11 direct browser automation, and 2026-08-12 via `server.js`/Kiro CLI from a different environment) produced **conflicting observations** on the following points. Both are presented on the relevant child page with their source and reasoning; neither has been re-verified as the definitive answer. **Flag these to the dev team for clarification rather than assuming either is correct:**

1. **Is "Personal"/"Business" a two-state toggle, or an add-policy action supporting multiple concurrent policies per life?** Session 1 found clicking Personal/Business each time creates a new, independently-numbered policy (Personal 1, Personal 2, Business 1, ...), with a working per-policy delete control, and the "Policies" badge counting up to 4 concurrently-existing policies. Session 2 described it as a simple two-state toggle where switching does not clear configured covers. See [Policy Structure — POL-05 through POL-09](quote-screen/policy-structure/page.md).
2. **What is the Business policy's 4th Lump Sum cover?** Session 1: Life, TPD, Trauma, **Specific Injury**. Session 2: Life, TPD, Trauma, **Cancer**. See [Lump Sum Covers — LSC-01](quote-screen/lump-sum-covers/page.md).
3. **Does the Business/Personal cover-menu split correlate with policy type, or with Occupation Code?** Session 1 tied the cover menu to which policy (Personal-type vs Business-type) is selected. Session 2's later, more granular test tied an equivalent-looking split directly to Occupation Code (AA gets one menu, all other codes get another) without varying policy type as an isolated variable. It is possible both factors matter and the two sessions were observing a confounded combination — this needs a clean test that holds one variable constant while varying the other. See [Policy Structure — POL-10](quote-screen/policy-structure/page.md) and [Lump Sum Covers — LSC-13](quote-screen/lump-sum-covers/page.md).
4. **Does switching Life tabs require the current life to meet a minimum bar?** Session 1 observed a blocking modal — *"Cannot proceed — Please enter the minimum requirement for a quote before proceeding to another life"* — appear during multi-life testing. Session 2's dedicated, isolated test of clicking "Add life" on a completely empty Life 1 found it succeeds unconditionally with no validation. These may not actually conflict (unconditional *creation* of a new life vs. a possible separate gate on *switching to* an already-existing incomplete one) — but this distinction hasn't been isolated. See [Policy Structure — POL-12](quote-screen/policy-structure/page.md).
