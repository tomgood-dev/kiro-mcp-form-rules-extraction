// Apply-flow reachability walk (single life). reachApplicationFlow -> Client Summary -> Proceed ->
// DoD -> Personal Details, then fill visible required fields + click Next, recording each screen,
// to see how far the flow goes (Occupation? Income? URE?). Error-swept.
...
function screen(page){return page.evaluate(()=>{function vis(e){return e&&e.offsetParent!==null;}
  var head=[].slice.call(document.querySelectorAll('h1,h2,[class*="title"],[class*="heading"]')).filter(vis).map(function(e){return (e.innerText||'').trim();}).filter(Boolean)[0]||'';
  var errs=[].slice.call(document.querySelectorAll('[class*="feedback"],[class*="error"],[role="alert"],.text-danger')).filter(vis).map(function(e){return (e.innerText||'').replace(/\s+/g,' ').trim();}).filter(function(t){return t&&t.toLowerCase()!=='remove'&&!/^(male female|yes no)$/i.test(t);});
  return {url:location.pathname, head:head.slice(0,40), errs:[...new Set(errs)].slice(0,5)};});}
...
(async()=>{process.env.NODE_TLS_REJECT_UNAUTHORIZED='0';
 ...
 try{
   // single-life to Client Summary, with occupation name to clear the gate
   ...
   log(Object.assign({step:'client summary'},await screen(q)));
   await H.proceedThroughClientSummary(q,{}).catch(()=>{});
   await settle(q,3000);
   log(Object.assign({step:'after proceed (expect DoD)'},await screen(q)));
   // Walk the wizard: accept DoD (Yes) then Next, recording each screen for up to 8 steps.
   for(var i=0;i<8;i++){
     await q.evaluate(()=>{var y=[].slice.call(document.querySelectorAll('button,.button-group-item,label')).find(function(b){return b.offsetParent!==null&&/^(yes|i agree|agree)$/i.test((b.innerText||'').trim());});if(y)y.click();});
     await settle(q,800);
     await q.evaluate(()=>{var n=[].slice.call(document.querySelectorAll('button,a')).find(function(b){return b.offsetParent!==null&&/^(next|continue|save and continue|proceed)/i.test((b.innerText||'').trim());});if(n)n.click();});
     await settle(q,3500);
     log(Object.assign({step:'wizard '+(i+1)},await screen(q)));
   }
   console.log('DONE');
