// Pass Personal Details: search HOME address, pick a real suggestion from the populated dropdown,
// tick "postal same as home = Yes", then Next. Dumps the address dropdown structure so we click a real item.
...
(async()=>{process.env.NODE_TLS_REJECT_UNAUTHORIZED='0';
 ...
 try{
   ...
   // fill the non-address required fields
   await q.evaluate(()=>{var t=document.getElementById('b5-Dropdown_Title');if(t){var o=[].slice.call(t.options).find(function(o){return /^mr$/i.test(o.text.trim());})||t.options[1];t.value=o.value;t.dispatchEvent(new Event('change',{bubbles:true}));}}); await settle(q,500);
   await q.evaluate(()=>{[].slice.call(document.querySelectorAll('.button-group-item')).filter(function(b){return b.offsetParent!==null&&/^No$/.test((b.innerText||'').trim());}).forEach(function(b){b.click();});}); await settle(q,500);
   ...
   // HOME address: real keyboard type, wait, DUMP the dropdown that appears, then click a real suggestion
   var addr=q.locator('[id="b5-b20-Input_AddressLookup"]');
   await addr.scrollIntoViewIfNeeded().catch(()=>{}); await addr.click().catch(()=>{});
   await q.keyboard.type('Queen Street', {delay:120}); await settle(q,4000);
   var dd=await q.evaluate(()=>{function vis(e){return e&&e.offsetParent!==null;}
     // find any list of options that appeared (exclude native <option>)
     var cands=[].slice.call(document.querySelectorAll('li,[role="option"],[class*="ListItem"],[class*="list-item"],[class*="dropdown"] a,[class*="dropdown"] div,[class*="autocomplete"] *,[class*="Result"] *')).filter(function(e){return vis(e)&&e.tagName!=='OPTION';}).map(function(e){return {tag:e.tagName, cls:String(e.className).slice(0,35), text:(e.innerText||'').replace(/\s+/g,' ').trim().slice(0,45)};}).filter(function(x){return x.text&&x.text.length>4&&/\d|street|road|ave|auckland|wellington/i.test(x.text);});
     return cands.slice(0,10);});
   log({homeAddressDropdown:dd});
   // click the first real suggestion
   var clicked=await q.evaluate(()=>{function vis(e){return e&&e.offsetParent!==null;}var o=[].slice.call(document.querySelectorAll('li,[role="option"],[class*="ListItem"],[class*="list-item"]')).filter(function(e){return vis(e)&&e.tagName!=='OPTION'&&/\d|street|road|auckland/i.test((e.innerText||''))&&(e.innerText||'').trim().length>4;})[0]; if(o){o.click();return (o.innerText||'').trim().slice(0,45);} return null;});
   log({homeAddressClicked:clicked}); await settle(q,2000);
   // tick "postal address same as home = Yes"
   await q.evaluate(()=>{function vis(e){return e&&e.offsetParent!==null;}
     // find a Yes button near a 'postal' / 'same as' label
     var groups=[].slice.call(document.querySelectorAll('.button-group-item,button')).filter(function(b){return vis(b)&&/^Yes$/.test((b.innerText||'').trim());});
     // heuristic: click the LAST Yes (postal question is below address)
     if(groups.length){groups[groups.length-1].click();}});
   await settle(q,1500);
   // Next
   await q.evaluate(()=>{var n=[].slice.call(document.querySelectorAll('button,a')).find(function(b){return b.offsetParent!==null&&/^(next|continue|save and continue)/i.test((b.innerText||'').trim());});if(n)n.click();}); await settle(q,5000);
