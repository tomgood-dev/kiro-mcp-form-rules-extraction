...
| AC18 | **Reachable — not yet wired (probed 2026-08-31, `evidence/16-probe-save-reopen-reachability/`).** A **Save** action exists on the quote screen and carries the chosen IC/RC + per-cover category (needs a fully-valid client). Reading it back requires reopening the saved quote from the dashboard "Quotes and Applications" list, whose rows open via a JS list-widget handler (no `QuoteId` URL) and load async — that row-open interaction is not yet cracked. Feasible, not blocked. |
...
| AC23, AC24, AC25 | **Reachable — not yet wired (probed, evidence 16).** The dashboard list contains real existing QUOTE and APPLICATION IN PROGRESS rows; opening one needs the same list-widget row-open interaction as AC18. Not blocked. |
