...
> Tested by: `apps/asteron-quote-apply/tests/quote-screen/select-default-commission-category-v1.spec.js`
> ([test doc](../../../test-documentation/select-default-commission-category-v1.md)) — consolidated
> 2026-08-27 from the earlier `part-1`/`part-2`/`part-3` split (now under `tests/deprecated/`),
> which itself replaced an original 7-part file (`comm-cat-v2.spec.js`) split for
> sustained-session-load reasons (see "Retracted findings" below).
...
> ✅ **Update, 2026-08-31 — the three regressions below are FIXED.** The dev team resolved
> `ADV-01`'s agency-number parenthetical, `ADV-07`'s Update-button-starts-disabled
> behavior, and `ADV-08`/`ADV-09`/`ADV-10`'s single-Upfront-valid-option auto-select —
> all confirmed passing again in a live re-run of the consolidated
> `select-default-commission-category-v1.spec.js` (see
> [test doc](../../../test-documentation/select-default-commission-category-v1.md), "Regression
> update 2026-08-31"). Original evidence for when these broke (2026-08-25/26, since
> superseded) is preserved in git history at
> `test-runs/select-default-commission-category-v1/2026-08-25T19-26-35/bug-reports/adviser-use-commission-regressions.md`.
> The rule text below now reflects confirmed-current-again behavior, not a still-open regression.
>
> ⚠ **But two NEW issues surfaced since, both still open:**
> 1. **AC06/AC08 — the agency default no longer persists across a fresh sign-out/sign-in.**
>    Different from the old AC07 issue (that confirmation-message bug is also now fixed) —
>    this is a genuinely new persistence failure. See the same test doc for the current
>    repro; not yet given its own Discrepancy Evidence Record here.
> 2. **AC13 — Adviser Use category-gating not enforced** for an invalid Spread20+2.5%
>    combination (auto-selects instead of forcing "Please Select"). See the Discrepancy
>    Evidence Record below.
>
> Neither is confirmed with a BA/PM as regression vs. intentional yet.

| Rule ID | Rule |
|---|---|
| `ADV-01` | Label reads **"Default for Agency (`<agency numbers>`)"** — a comma-separated list of real agency numbers, confirming AC01. *(Regressed 2026-08-25, fixed by 2026-08-31 — see callout above.)* |
...
| `ADV-04` | When **Flexi Rate = N/A**, the **Select IC/RC** dropdown for a cover has exactly one real option (`IC-100%, RC-100%`) besides "Please Select", and it is **auto-selected** rather than left on "Please Select" (AC14). Never affected by the ADV-08/09/10 regression below — confirmed working throughout. |
...
| `ADV-07` | **Update** button starts **disabled**, and only enables once the Default for Agency selection genuinely differs from the currently-saved value; reverting the selection back to the saved value re-disables it (AC04, AC05, and the spirit of AC09). See "Retracted findings" below for the 2026-08-19 false-positive report of this same symptom — that one was a `page.mouse.wheel()` artifact. A second, genuine regression of the same symptom (button starting enabled) occurred 2026-08-25 and was independently re-confirmed with no such interaction — but was **fixed by 2026-08-31** (see callout above), confirmed via a live re-run of AC04/AC05/AC09. |
| `ADV-08` | **Example 1 (2.5% Flexi Rate):** Select IC/RC auto-selects `IC-100%, RC-50%` (the documented Upfront default, the only valid option — AC14); the per-cover "Life Cover" row auto-selects `Upfront`. Confirmed via `evidence/06-probe-example1-category-default/`. *(Regressed 2026-08-25, fixed by 2026-08-31 — see callout above.)* |
| `ADV-09` | **Example 2 (7.5% Flexi Rate):** Select IC/RC auto-selects `IC-75%, RC-100%` (the documented Upfront default, the only valid option — AC14), with exactly the 4 documented options. Confirmed via `evidence/08-probe-clean-single-flexirate-7.5pct/` — see "Retracted findings" for why this took two attempts. *(Regressed 2026-08-25, fixed by 2026-08-31 — see callout above.)* |
| `ADV-10` | **Example 3 (15% Flexi Rate):** Select IC/RC auto-selects `IC-50%, RC-50%` (the documented Upfront default, the only valid option — AC14); the per-cover "Life Cover" row auto-selects `Upfront`. Confirmed via `evidence/07-probe-examples-3-4/`. *(Regressed 2026-08-25, fixed by 2026-08-31 — see callout above.)* |
| `ADV-11` | **Example 4 (12.5% Flexi Rate, multiple UPFRONT IC/RC rates):** Select IC/RC correctly stays on `Please Select` (does NOT auto-select) with exactly the 4 documented options, since multiple valid IC/RC combinations exist for Upfront at this rate — matches the doc's explicit "the adviser must select an option" instruction and the general principle behind AC15. Confirmed via `evidence/09-probe-clean-single-flexirate-12.5pct/` — see "Retracted findings" for why this took two attempts. Never affected by the ADV-08/09/10 regression — confirmed working throughout, including during the window when those three were broken (2026-08-25 to 2026-08-31), which helped narrow that regression to specifically "pick the one Upfront-valid option among several listed," not the whole auto-select mechanism. |
