...
| AC(s) | Reason |
|-------|--------|
| AC02, AC04 | Reachable flow, but the answer-mutation path is **walled** (characterized 2026-09-15, `probe-ure-untick-2026-09-15.js`): once the URE questionnaire is complete, navigating back lands on the RESIDENCE AND TRAVEL summary page, and footer **Previous** from there EXITS the Personal Statement (to Tele Interview) rather than paging back through the internal health pages — so the individual health questions aren't re-exposed to flip a No→Yes and spawn follow-ups. AC02/AC04 need the questionnaire's own internal edit/re-open affordance (not yet located) to reach a specific health question. `getApplyFlowSidebar()` already reads the tick state; only the re-open mechanism is missing. Two approaches tried before deferring. |
