/**
 * Commission Category (Adviser Use / Commissions modal) — v2, full-coverage of the
...
test('Commission category v2: fresh-quote-per-Flexi-Rate coverage of named examples', async ({ page }) => {
  try {
    ...
    console.log('[comm-cat-v2] Login OK, ' + new Date().toISOString());
    ...
    async function openFreshQuote() {
      console.log('[comm-cat-v2]   openFreshQuote: navigating to quote list...');
      ...
      console.log('[comm-cat-v2]   openFreshQuote: clicking New Quote...');
      ...
      // window.open() can be called with a relative path (observed with this account) or an
      // absolute URL (observed with the original account) - normalize before navigating,
      // since page.goto() requires an absolute URL.
    ...
    async function setGender(gender) {
      ...
      // control (3rd of 7 fresh quotes in one run) - retry a couple times before failing
      // for real, rather than a hard fail-fast on the first flaky click.
      for (var attempt = 1; attempt <= 3; attempt++) {
        ...
        if (selected) return;
        console.log('[comm-cat-v2]   setGender: attempt ' + attempt + ' did not register, retrying...');
      }
    ...
    async function freshPricedQuote() {
      console.log('[comm-cat-v2]   freshPricedQuote: opening new quote...');
      await openFreshQuote();
      console.log('[comm-cat-v2]   freshPricedQuote: setting age...');
      await setAge('35');
      console.log('[comm-cat-v2]   freshPricedQuote: setting gender...');
      await setGender('Male');
      console.log('[comm-cat-v2]   freshPricedQuote: setting occupation code...');
      await setOCC('1'); // AA
      console.log('[comm-cat-v2]   freshPricedQuote: activating Life cover...');
      await activateCover('Life');
      console.log('[comm-cat-v2]   freshPricedQuote: entering Sum Insured...');
      ...
      console.log('[comm-cat-v2]   freshPricedQuote: priced OK (' + premium.replace(/\s+/g, ' ').trim() + ')');
    }
    ...
    console.log('[comm-cat-v2] PART 1 starting: Default for Agency display + AC14 (N/A auto-select)');
    ...
    console.log('[comm-cat-v2] PART 1 PASSED');
    ...
    console.log('[comm-cat-v2] PART 2 starting: 30% Flexi Rate forces Nil Commission');
    await freshPricedQuote();
    ...
    await closeAdviserUse();
    console.log('[comm-cat-v2] PART 2 PASSED');
    ...
    console.log('[comm-cat-v2] PART 3 starting: Update button disabled-until-changed');
    await freshPricedQuote();
    await openAdviserUse();
    ...
    await closeAdviserUse();
    console.log('[comm-cat-v2] PART 3 PASSED');
    ...
    console.log('[comm-cat-v2] PART 4 starting: Example 1, Flexi Rate 2.5%');
    await freshPricedQuote();
    ...
    await closeAdviserUse();
    console.log('[comm-cat-v2] PART 4 PASSED');

    // ════════════════════════════════════════════════════════════════
    // PART 5: EXAMPLE 2 - FLEXI RATE 7.5% (ADV-09)
    // Fresh quote #5. v1 got this wrong (IC-100%,RC-100% observed) because it reused
    // a quote already opened at Flexi Rate N/A - see file header.
    // ════════════════════════════════════════════════════════════════

    console.log('[comm-cat-v2] PART 5 starting: Example 2, Flexi Rate 7.5%');
    await freshPricedQuote();
    ...
    await closeAdviserUse();
    console.log('[comm-cat-v2] PART 5 PASSED');
    ...
    console.log('[comm-cat-v2] PART 6 starting: Example 3, Flexi Rate 15%');
    await freshPricedQuote();
    ...
    await closeAdviserUse();
    console.log('[comm-cat-v2] PART 6 PASSED');

    // ════════════════════════════════════════════════════════════════
    // PART 7: EXAMPLE 4 - FLEXI RATE 12.5%, MULTIPLE UPFRONT IC/RC RATES (ADV-11)
    // Fresh quote #7. v1 got this wrong (a 5th phantom option auto-selected) because
    // it tested this immediately after Example 3 in the same quote - see file header.
    // ════════════════════════════════════════════════════════════════

    console.log('[comm-cat-v2] PART 7 starting: Example 4, Flexi Rate 12.5%');
    await freshPricedQuote();
    ...
    await closeAdviserUse();
    console.log('[comm-cat-v2] PART 7 PASSED - ALL PARTS PASSED');
