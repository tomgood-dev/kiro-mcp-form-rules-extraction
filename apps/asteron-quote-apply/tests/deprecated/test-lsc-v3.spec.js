 ...
 * LSC-20: Major Trauma $2M ceiling
...
test('LSC-32 companion cover + LSC-19 Major Trauma 300% + LSC-20 $2M ceiling', async ({ page }) => {
  try {
    ...
    await majorSI.click();
    await page.waitForTimeout(200);
    for (var p = 0; p < 12; p++) await page.keyboard.press('Backspace');
    await page.waitForTimeout(200);
    var digits3 = '60001';
    for (var q = 0; q < digits3.length; q++) { await page.keyboard.press(digits3[q]); await page.waitForTimeout(60); }
    await page.keyboard.press('Tab');
    await page.locator('text=Loading').first().waitFor({ state: 'hidden', timeout: 15000 }).catch(function() {});
    await page.waitForTimeout(2000);
    ...
    await traumaSI.click();
    await page.waitForTimeout(200);
    for (var r = 0; r < 12; r++) await page.keyboard.press('Backspace');
    await page.waitForTimeout(200);
    var digits4 = '25000';
    for (var s = 0; s < digits4.length; s++) { await page.keyboard.press(digits4[s]); await page.waitForTimeout(60); }
    await page.keyboard.press('Tab');
    await page.locator('text=Loading').first().waitFor({ state: 'hidden', timeout: 15000 }).catch(function() {});
    await page.waitForTimeout(2000);
    ...
    await majorSI.click();
    await page.waitForTimeout(200);
    for (var t = 0; t < 12; t++) await page.keyboard.press('Backspace');
    await page.waitForTimeout(200);
    var digits5 = '1975001';
    for (var u = 0; u < digits5.length; u++) { await page.keyboard.press(digits5[u]); await page.waitForTimeout(60); }
    await page.keyboard.press('Tab');
    await page.locator('text=Loading').first().waitFor({ state: 'hidden', timeout: 15000 }).catch(function() {});
    await page.waitForTimeout(2000);

    var errors3 = await page.evaluate(function() {
      var nodes = Array.from(document.querySelectorAll('[class*="error"], [class*="Error"], [class*="background-error"]'));
      return nodes.filter(function(n) { return n.innerText && n.getBoundingClientRect().width > 0; }).map(function(n) { return n.innerText.trim(); });
    });
    // Should NOT have the 300% error (no percentage cap at $25k+)
    var has300Error = errors3.some(function(e) { return e.includes('maximum Sum Insured for Major Trauma Benefit based on'); });
    if (has300Error)
