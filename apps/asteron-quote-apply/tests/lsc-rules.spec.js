...
test('Lump Sum Cover Rules (LSC-19, LSC-32)', async ({ page }) => {
  try {
    ...
    if (!BASE_URL) throw new Error('ENV: BASE_URL is not set. Value: "' + process.env.BASE_URL + '"');
    if (!LOGIN_EMAIL) throw new Error('ENV: LOGIN_EMAIL is not set. Value: "' + process.env.LOGIN_EMAIL + '"');
    if (!LOGIN_PASSWORD) throw new Error('ENV: LOGIN_PASSWORD is not set. Value: "' + process.env.LOGIN_PASSWORD + '"');
    ...
    if (page.url().includes('_error.html')) {
      throw new Error('LOGIN PAGE BLOCKED: ' + page.url());
    }

    const emailField = page.locator('input[type="text"]').first();
    if (!(await emailField.isVisible().catch(() => false))) {
      throw new Error('LOGIN FORM NOT RENDERED. Current URL: ' + page.url());
    }
    ...
    if (page.url().includes('CentralPortalsLogin')) {
      throw new Error('LOGIN FAILED: Still on login page after 30s. URL: ' + page.url());
    }
    ...
    // Re-throw with full details so the Test Suite shows it
