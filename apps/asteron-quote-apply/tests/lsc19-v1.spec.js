/**
 * LSC-19: Major Trauma 300% cap + LSC-20: $2M ceiling
...
test('LSC-19 Major Trauma 300% cap + LSC-20 $2M ceiling', async ({ page }) => {
  try {
    ...
    var d1 = '20000'; for (var k = 0; k < d1.length; k++) { await page.keyboard.press(d1[k]); await page.waitForTimeout(60); }
    ...
    var d2 = '60001'; for (var n = 0; n < d2.length; n++) { await page.keyboard.press(d2[n]); await page.waitForTimeout(60); }
    await page.keyboard.press('Tab');
    await page.locator('text=Loading').first().waitFor({ state: 'hidden', timeout: 15000 }).catch(function() {});
    await page.waitForTimeout(2000);

    // LSC-19: Check 300% cap error
    ...
    // LSC-20: Change Trauma to $25k, Major to $1,975,001
    await traumaSI.scrollIntoViewIfNeeded(); await traumaSI.click(); await page.waitForTimeout(200);
    for (var p = 0; p < 12; p++) await page.keyboard.press('Backspace'); await page.waitForTimeout(200);
    var d3 = '25000'; for (var q = 0; q < d3.length; q++) { await page.keyboard.press(d3[q]); await page.waitForTimeout(60); }
    await page.keyboard.press('Tab');
    await page.locator('text=Loading').first().waitFor({ state: 'hidden', timeout: 15000 }).catch(function() {});
    await page.waitForTimeout(2000);
    ...
    var d4 = '1975001'; for (var s = 0; s < d4.length; s++) { await page.keyboard.press(d4[s]); await page.waitForTimeout(60); }
    await page.keyboard.press('Tab');
    await page.locator('text=Loading').first().waitFor({ state: 'hidden', timeout: 15000 }).catch(function() {});
    await page.waitForTimeout(2000);
