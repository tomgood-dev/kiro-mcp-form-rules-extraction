...
test.describe('Multi Lives and Policies (ACB-4394)', () => {
  ...
  test('MLP-20/AC20: Start Application proceeds, shows Continue Application on return', async ({ page }) => {
    ...
    test.fixme(true, 'Deferred (retested 2026-09-16): same blocker chain as MLP-11. Single-life submits end-to-end now, but the multi-life path needs (a) a hardened 2-life build to reliably reach Client Summary (the retest build stayed on the Quote screen, proceedBtns=0 - probe-mlp11-retest-2026-09-16.js) and then (b) confirmation that per-life Proceed navigates. The Start->Continue-on-return status transition can only be tested once per-life app-entry works. Client Summary + per-life controls/status are covered by MLP-10/MLP-19.');
  });
