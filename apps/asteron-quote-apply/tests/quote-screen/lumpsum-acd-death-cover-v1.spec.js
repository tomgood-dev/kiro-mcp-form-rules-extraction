...
test.describe('Lumpsum Acd. Death Cover (ACB-2929)', () => {
  ...
  test('AC01/AC02: All lump sum covers available; 1+ selectable', async ({ page }, testInfo) => {
    ...
    await recordStep(testInfo, page, { label: 'Acd. Death is selectable (its Sum Insured field appears)', expected: true, actual: siVisible });
  ...
  test('AC03: Acd. Death exposes SI + Premium Structure greyed out and fixed to Stepped', async ({ page }, testInfo) => {
    ...
    const siVisible = await sumInsuredInput(quote, 0).isVisible();
    await recordStep(testInfo, page, { label: 'Acd. Death Sum Insured field is present', expected: true, actual: siVisible });
    expect(siVisible, 'AC03: Sum Insured field present').toBe(true);
    const struct = await getAcdStructure(quote);
    await recordStep(testInfo, page, { label: 'Acd. Death Premium Structure value', expected: 'Stepped', actual: struct?.selected });
    expect(struct?.selected, 'AC03: Premium Structure fixed to Stepped').toBe('Stepped');
    await recordStep(testInfo, page, { label: 'Acd. Death Premium Structure is greyed out (disabled)', expected: true, actual: struct?.disabled });
  ...
  test('AC04: Acd. Death + ANB > 70 → maximum age error', async ({ page }, testInfo) => {
    ...
    await recordStep(testInfo, page, { label: 'Error shown for Acd. Death ANB > 70', expected: 'maximum Age Next Birthday for Accidental Death Cover is 70', actual: e });
  ...
  test('AC04 boundary: Acd. Death max age at ANB 70 is accepted (no max-age error)', async ({ page }, testInfo) => {
    ...
    await recordStep(testInfo, page, { label: 'Acd. Death max age at ANB 70 accepted (no max-age error)', expected: false, actual: hasErr });
  ...
  test('AC05: Acd. Death + ANB < 17 → minimum age error', async ({ page }, testInfo) => {
    ...
    await clickApply(quote);
    const e = await errText(quote);
    await recordStep(testInfo, page, { label: 'Error shown for Acd. Death ANB < 17', expected: 'minimum Age Next Birthday for Accidental Death Cover is 17', actual: e });
  ...
  test('AC05 boundary: Acd. Death min age at ANB 17 is accepted (no min-age error)', async ({ page }, testInfo) => {
    ...
    await recordStep(testInfo, page, { label: 'Acd. Death min age at ANB 17 accepted (no min-age error)', expected: false, actual: hasErr });
  ...
  test('AC06: Acd. Death + SI > $1,000,000 → maximum SI cap error', async ({ page }, testInfo) => {
    ...
    await clickApply(quote);
    const e = await errText(quote);
    await recordStep(testInfo, page, { label: 'Error shown for Acd. Death SI > $1,000,000', expected: 'maximum sum insured for Accidental Death Cover is $1,000,000', actual: e });
  ...
  test('AC06 boundary: Acd. Death SI exactly $1,000,000 is accepted (no cap error)', async ({ page }, testInfo) => {
    ...
    await recordStep(testInfo, page, { label: 'Acd. Death SI exactly $1,000,000 accepted (no cap error)', expected: false, actual: hasCap });
  ...
  test('AC07: Acd. Death cover can be added and removed, premium reflects it', async ({ page }, testInfo) => {
    ...
    await recordStep(testInfo, page, { label: 'Acd. Death Sum Insured field present after adding the cover', expected: true, actual: presentAfterAdd });
    ...
    await recordStep(testInfo, page, { label: 'Acd. Death Sum Insured field removed after removing the cover', expected: 0, actual: siCountAfterRemove });
  ...
  test('AC08: Sum Insured "?" tooltip shows the Accidental Death discount-bands text', async ({ page }, testInfo) => {
    ...
    await recordStep(testInfo, page, { label: 'Acd. Death Sum Insured tooltip lists the discount bands', expected: 'contains $150,000-$249,999 ... $1,000,000', actual: bandText });
