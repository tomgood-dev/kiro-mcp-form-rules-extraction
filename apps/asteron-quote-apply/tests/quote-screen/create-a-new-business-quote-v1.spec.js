...
test.describe('Create a New Business Quote (ACB-2240)', () => {
  ...
  test.fixme('AC01: Landing page — select agency, then click create quote', async ({ page }) => {
  ...
  test('AC02: select Personal and/or Business Policy on a new quote', async ({ page }) => {
    test.info().annotations.push({ type: 'acceptance-criteria', description: [
  ...
  test('AC04a: Personal Details fields are present on the new quote screen', async ({ page }) => {
    test.info().annotations.push({ type: 'acceptance-criteria', description: [
  ...
  test('AC04b: Lump Sum Cover types and sub-covers are present (Personal policy)', async ({ page }) => {
    test.info().annotations.push({ type: 'acceptance-criteria', description: [
    ...
    for (const cover of ['Life', 'TPD', 'Trauma', 'Cancer', 'Acd. Death', 'Needlestick', 'Specific Injury']) {
      expect(await coverButtonExists(quote, cover), `AC04: top-level cover "${cover}" present`).toBe(true);
    }
    await activateCover(quote, 'Life');
    await fillCalcMask(sumInsuredInput(quote, 0), '200000');
    for (const subcover of ['TI Support', 'Acc. TPD', 'Acc. Trauma', 'Acc. Cancer']) {
      expect(await coverButtonExists(quote, subcover), `AC04: Life sub-cover "${subcover}" present`).toBe(true);
    }
    ...
    expect(await coverButtonExists(quote, 'Major Trauma'), 'AC04: "Major Trauma" sub-cover present').toBe(true);
    expect(await coverButtonExists(quote, 'TPD on Trauma'), 'AC04: "TPD on Trauma" sub-cover present').toBe(true);
  });

  test('AC04c: Disability Covers and Kids Cover are present', async ({ page }) => {
    test.info().annotations.push({ type: 'acceptance-criteria', description: [
    ...
    for (const cover of ['Mortgage & Living', 'Income Protection', 'Workability']) {
      expect(await coverButtonExists(quote, cover), `AC04: Disability cover "${cover}" present`).toBe(true);
    }
  ...
  test('AC05: entering Date of Birth calculates and displays Age Next Birthday', async ({ page }) => {
    test.info().annotations.push({ type: 'acceptance-criteria', description: [
  ...
  test('AC06: selecting Occupation (type-ahead) prepopulates Occupation Code', async ({ page }) => {
    test.info().annotations.push({ type: 'acceptance-criteria', description: [
  ...
  test('AC07: selecting Occupation Code instead prepopulates the Occupation field', async ({ page }) => {
    test.info().annotations.push({ type: 'acceptance-criteria', description: [
  ...
  test('AC08: Employment status offers exactly the 4 documented options', async ({ page }) => {
    test.info().annotations.push({ type: 'acceptance-criteria', description: [
  ...
  test('AC09c: Smoker always has a default value (no reachable "unset" state)', async ({ page }) => {
    test.info().annotations.push({ type: 'acceptance-criteria', description: [
  ...
  test('AC10: Flexi Rate list runs N/A then 2.5% to 30.00% in 2.5% steps', async ({ page }) => {
    test.info().annotations.push({ type: 'acceptance-criteria', description: [
  ...
  test('AC11: We Pay Your Premiums warns when no lump sum cover is selected', async ({ page }) => {
    test.info().annotations.push({ type: 'acceptance-criteria', description: [
  ...
  test('AC12: selecting a cover type exposes Sum Insured + Premium Structure (default Stepped)', async ({ page }) => {
    test.info().annotations.push({ type: 'acceptance-criteria', description: [
    ...
    expect(await getPremiumStructure(quote), 'AC12: Premium Structure defaults to Stepped').toBe('Stepped');
  });

  test('AC13: adding/removing a cover is reflected in the premium panel', async ({ page }) => {
    test.info().annotations.push({ type: 'acceptance-criteria', description: [
  ...
  test('AC14: Kids Cover — number of kids drives per-kid fields and the SI tier default', async ({ page }) => {
    test.info().annotations.push({ type: 'acceptance-criteria', description: [
    ...
    await waitForSettle(quote, 1500);
    expect(await quote.locator('input[type="date"]').count(), 'AC14: a per-kid Date of birth field appears').toBeGreaterThan(1); // the adult DOB + at least one kid DOB
  ...
  test('AC15/BR-006: Payment Frequency defaults to Monthly, full 5-option list, independent per policy', async ({ page }) => {
    test.info().annotations.push({ type: 'acceptance-criteria', description: [
    ...
    expect(await freqSelects.count(), 'AC15/BR-006: 2 independent Payment Frequency selects (Personal + Business)').toBe(2);
    for (let i = 0; i < 2; i++) {
  ...
  test('BR-004: "Add life" creates an additional, independent life on the quote', async ({ page }) => {
    test.info().annotations.push({ type: 'acceptance-criteria', description: [
