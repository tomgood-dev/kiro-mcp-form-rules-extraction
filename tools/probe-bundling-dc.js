/**
 * Probe: check Bundling Discounts text when M&L is active
 */
...
(async () => {
  ...
  try {
    ...
    // Activate Life $200k
    await page.evaluate(() => { const btn = [...document.querySelectorAll('button')].find(b => b.innerText.trim() === 'Life'); if (btn) btn.click(); });
    await page.waitForTimeout(3000);
    const lifeSI = page.locator('input[id*="Input_SumInsured"]').first();
    await lifeSI.scrollIntoViewIfNeeded(); await lifeSI.click(); await page.waitForTimeout(200);
    ...
    // Activate M&L and commit
    await page.evaluate(() => { const btn = [...document.querySelectorAll('button')].find(b => b.innerText.trim() === 'Mortgage & Living'); if (btn) btn.click(); });
    await page.waitForTimeout(3000);
    const mlSI = page.locator('input[id*="Input_SumInsured"]').nth(1);
    await mlSI.scrollIntoViewIfNeeded(); await mlSI.click();
    await page.keyboard.press('Tab'); await page.waitForTimeout(3000);

    // Check bundling text
    const bundlingText = await page.evaluate(() => {
      const text = document.body.innerText;
      const idx = text.indexOf('Bundling');
      if (idx === -1) return 'NOT FOUND anywhere in body';
      return text.slice(Math.max(0, idx - 20), idx + 100).replace(/\n/g, ' | ');
    });
    console.log('After Life + M&L committed:');
    console.log('  Bundling text: ' + bundlingText);

    // Now add TPD
    await page.evaluate(() => { const btn = [...document.querySelectorAll('button')].find(b => b.innerText.trim() === 'TPD'); if (btn) btn.click(); });
    await page.waitForTimeout(3000);
    const siCount = await page.evaluate(() => document.querySelectorAll('input[id*="Input_SumInsured"]').length);
    console.log('  SI field count after TPD: ' + siCount);

    const tpdField = page.locator('input[id*="Input_SumInsured"]').nth(siCount - 1);
    ...
    const bundlingText2 = await page.evaluate(() => {
      const text = document.body.innerText;
      const idx = text.indexOf('Bundling');
      if (idx === -1) return 'NOT FOUND anywhere in body';
      return text.slice(Math.max(0, idx - 20), idx + 100).replace(/\n/g, ' | ');
    });
    console.log('After Life + M&L + TPD $100k:');
    console.log('  Bundling text: ' + bundlingText2);
